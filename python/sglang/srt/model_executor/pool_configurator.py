# ENGRAM_MODIFIED — pure-SSM memory pool configurator
...
# --- BEGIN ENGRAM: pure-SSM memory pool configurator ---
def _is_pure_ssm(mr: ModelRunner) -> bool:
    """True iff model has Mamba state but no attention layers in this shard.

    Pure SSM models (e.g., Codestral Mamba 7B) have no MHA/MLA layers, so the
    KV token pool has zero cell_size and upstream's DefaultPoolConfigurator
    would divide by zero. Mamba state cache is the real capacity limiter.
    """
    cfg = mr.mambaish_config
    if cfg is None:
        return False
...
class PureSSMMemoryPoolConfigurator(MemoryPoolConfigurator):
    """Pure-SSM models: KV token pool is not the memory bottleneck — the Mamba
    state cache is, enforced via max_mamba_cache_size in _resolve_max_num_reqs.
    FIXED_SLOTS = 1<<20 is a "large enough" sentinel so the token-pool allocator
    succeeds without imposing a real cap.

    Pre-refactor (fork's old _profile_available_bytes Block 1) this path
    returned rest_memory // (context_len * 2), a bf16-activations heuristic.
    That formula was a relic of upstream's old tokens-and-cells coupling.
    Upstream's refactor cleanly separates bytes-profiling from token derivation,
    which makes the old formula arithmetically unprincipled for cell_size==0
    (it was computing "slots" based on unused KV byte math).
    Block 2 of fork's old code already agreed with the 1<<20 sentinel approach
    for the same branch — it was unreachable due to Block 1's early return,
    but its rationale is what we port forward here.
    """

    FIXED_SLOTS = 1 << 20  # 1M: sentinel for "not the bottleneck"

    def __init__(self, mr: ModelRunner):
        # No per-configurator state needed. max_mamba_cache_size is set
        # by handle_max_mamba_cache() already fired during
        # _profile_available_bytes when mambaish_config is not None.
        pass

    def calculate_pool_sizes(
        self, available_bytes: int, page_size: int
    ) -> MemoryPoolConfig:
        tokens = (self.FIXED_SLOTS // page_size) * page_size
        return MemoryPoolConfig(max_total_num_tokens=tokens)

    def calculate_pool_sizes_from_max_tokens(
        self, max_total_num_tokens: int, page_size: int
    ) -> MemoryPoolConfig:
        tokens = (max_total_num_tokens // page_size) * page_size
        return MemoryPoolConfig(max_total_num_tokens=tokens)
...
def create_memory_pool_configurator(
    ...
    # --- BEGIN ENGRAM: pure-SSM dispatch ---
    if _is_pure_ssm(mr):
        return PureSSMMemoryPoolConfigurator(mr)
    # --- END ENGRAM ---
