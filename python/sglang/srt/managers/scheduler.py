...
class Scheduler(
    ...
    # --- BEGIN ENGRAM: snapshot system lifecycle delegators ---
    def init_snapshot_system(self):
        """Initialize the Mamba state snapshot system for stateful inference.

        Delegates to ``scheduler_snapshot_system.init_snapshot_system``. The
        Scheduler continues to own the resulting snapshot/tier attributes
        (``self.snapshot_manager`` etc.) set by the helper.
        """
        _snapshot_system.init_snapshot_system(self)
    ...
    handle_restore_snapshot = _snapshot_handlers.handle_restore_snapshot
    handle_delete_snapshot = _snapshot_handlers.handle_delete_snapshot
    handle_mamba_state_eviction = _snapshot_handlers.handle_mamba_state_eviction
    # --- END ENGRAM ---

    # --- BEGIN ENGRAM: agent tool framework delegator ---
    def init_agent_system(self):
        """Initialize the agent framework for tool-calling support.

        Delegates to ``scheduler_agent_system.init_agent_system``. The
        Scheduler continues to own ``tool_registry``, ``tool_parser``,
