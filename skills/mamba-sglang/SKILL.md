---
name: mamba-sglang
description: Customized Mamba-SGLang fork featuring stateful Mamba snapshot capabilities for branching and rollback, alongside vanilla SGLang's fast structured generation. Fully backward compatible with standard SGLang.
version: 1.0.0
author: Orchestra Research
license: MIT
tags: [Inference Serving, SGLang, Structured Generation, RadixAttention, Prefix Caching, Constrained Decoding, Agents, JSON Output, Fast Inference, Production Scale, Mamba, State Snapshots, Conversation Branching]
dependencies: [sglang, torch, transformers]
---

# Mamba-SGLang (Stateful Customization)

> **Agent Note**: This is a customized fork (`mamba-sglang`) featuring stateful snapshot capabilities for Mamba models. It is fully backward compatible with all standard SGLang features.

High-performance serving framework for LLMs and VLMs with RadixAttention for automatic prefix caching, extended with robust Mamba state management.
...
- Stateful generation with save/restore points for branching conversations and state rollback
...
# With Stateful Mamba Snapshots (Customized)
python -m sglang.launch_server \
...
## Stateful Mamba Snapshots (Customized)

**What it does**: Allows users to save Mamba architecture conversation state at any exact point and restore it later.

**Key Capabilities**:
- **Branching Conversations**: Save state at a fork and explore multiple paths independently.
- **State Rollback**: Undo latest turns without re-computing past states.
- **Persistent Memory**: Manage snapshots on disk with high-level Python APIs.
